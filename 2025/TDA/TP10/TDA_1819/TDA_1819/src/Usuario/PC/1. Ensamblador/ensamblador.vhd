	...
	BEGIN
		...
		if (match) then
			...
			if (INSTTD_SIZE = 6) then
				...
				--- Añado lógica para detectar numeros literales como Inm ---		
				    -- debe haber al menos un dígito
				    if (not isNumber(cadena(indice))) then
				        report "Error en la línea " & integer'image(num_linea) &
				               " del programa '" & trim(nombre) &
				               "': el segundo operando no coincide con una variable ni es un inmediato válido"
				        severity FAILURE;
				    end if;
				
				    -- parsear número decimal de varias cifras
				    addrInm := 0;
				    while (indice <= cadena'length and isNumber(cadena(indice))) loop
				        for j in DIGITS_DEC'range loop
				            if (cadena(indice) = DIGITS_DEC(j)) then
				...
				indice := indice + 1; 
				--- Añado lógica para detectar SP como registro fuente ---
				if (cadena(indice) = 's') then
					indice := indice +1;
					if (cadena(indice) /= 'p') then
						report "Error en la línea " & integer'image(num_linea) & " del programa '" & trim(nombre) & "': el segundo operando se encuentra incorrectamente declarado"
						severity FAILURE;
					end if;
					indice := indice + 1;
					...
					addrReg := ID_SP;
					-- SP apunta en la prox direccion vacia de la pila a escribir, por eso sumo 1
				    addrInm := addrInm +1;
				else 
					if (cadena(indice) /= 'r') then
					report "Error en la línea " & integer'image(num_linea) & " del programa '" & trim(nombre) & "': el segundo operando se encuentra incorrectamente declarado"
