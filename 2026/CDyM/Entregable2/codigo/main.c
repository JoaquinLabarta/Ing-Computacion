...
static void Ports_Init(void)
{
	DDRB = 0x00;
	DDRC = 0x00;
	DDRD = 0x00; // Todos los pines como entrada

	PORTB = 0x00;
	PORTC = 0x00;
	PORTD = 0x00; // En 0 la salidas
...
static void teclado_Init(void)
{
	...
	DDRD |= (1 << teclado_FILA3_PIN); // Salida
...
static void teclado_Actualizar(void)
{
	...
	if (teclado_Escanear(&tecla_presionada) == true)
	{ // tecla nueva
...
static char teclado_LeerMatriz(void)
{
	...
	for (fila = 0; fila < 4; fila++)
	{
		...
		{
			if (teclado_IsColumnaLow(columna) != false)
			{
				found_tecla = keymap[fila][columna];
				break;
			}
		}
...
static void teclado_FilaLow(uint8_t row)
{
	...
	{
	case 0:
		PORTB &= ~(1 << teclado_FILA0_PIN);
		break;

	case 1:
		PORTB &= ~(1 << teclado_FILA1_PIN);
		break;

	case 2:
		PORTB &= ~(1 << teclado_FILA2_PIN);
		break;
...
static uint8_t teclado_IsColumnaLow(uint8_t column)
{
	...
	{
	case 0:
		return ((PIND & (1 << teclado_COL0_PIN)) == 0) ? true : false;

	case 1:
		return ((PIND & (1 << teclado_COL1_PIN)) == 0) ? true : false;

	case 2:
		return ((PIND & (1 << teclado_COL2_PIN)) == 0) ? true : false;

	case 3:
		return ((PIND & (1 << teclado_COL3_PIN)) == 0) ? true : false;
...
static void micro_ProcesarTecla(char tecla)
{
	if ((tecla >= '0') && (tecla <= '9'))
	{
		...
		{
			micro_IngresarDigito((uint8_t)(tecla - '0'));
		}
		return;
	}
	...
	{
	case 'A':
		if ((Estado_sistema == LIBRE) || (Estado_sistema == PAUSADO))
		{
...
static void micro_CargarDigitos(uint16_t total_segundos)
{
	...
	if (total_segundos > MAX_TIEMPO_SEGUNDOS)
	{
		total_segundos = MAX_TIEMPO_SEGUNDOS;
	}
...
static void micro_EmpezarCoccion(void)
{
	...
	{
		micro_Escribir(MSG_PUERTA_ABIERTA);
		return;
	}

	if (Tiempo_coccion_segundos == 0)
	{
		micro_Escribir(MSG_TIEMPO_CERO);
		return;
	}
...
static void micro_ActualizarLeds(void)
{
	if ((Estado_sistema == COCINANDO) && (Puerta_abierta == false))
	{
		PORTB |= (1 << LED_MAGNETRON_PIN);
	}
	else
	...
	if ((Estado_sistema == COCINANDO) || (Estado_sistema == PAUSADO) || (Puerta_abierta == true))
	{
	...
	if ((Estado_sistema == TERMINADO) && (Parpadeo == true))
	{
...
static void micro_ActualizarDisplay(void)
{
	...
	if ((Estado_sistema == TERMINADO) && (Parpadeo == false))
	{
	...
	if (Mensaje_ticks_100ms != 0)
	{
		if (Mensaje_code == MSG_PUERTA_ABIERTA)
		{
			line2 = "PUERTA ABIERTA";
		}
		else if (Mensaje_code == MSG_TIEMPO_CERO)
		{
			line2 = "TIEMPO 00:00";
		}
		else if (Mensaje_code == MSG_INVALIDO)
		{
			line2 = "SEGUNDOS > 59";
		}
		else
